...
- Language: Java
- https://spring.io/projects/spring-boot[Spring Boot]: a version compatible with the Spring Data Aerospike used by this repository
- Java: 17
...
In this demo repository, the canonical Spring Data Aerospike version is the one configured in the root
link:../pom.xml[`pom.xml`]. See the `Version Compatibility` section in the root link:../README.adoc[README] for the
...
(see more https://aerospike.com/docs/reference/limitations[limitations]).
...
Spring Data can use all-args constructor instead of reflection to https://docs.spring.io/spring-data/commons/reference/object-mapping.html#mapping.object-creation[gain performance boost] for object creation.
...
<8> `@Indexed` creates https://aerospike.com/docs/server/architecture/secondary-index[secondary index] on application startup for the specific field. Note: `@Indexed` annotation is not supported for the fields annotated with `@Id`, `@Expiration` or `@Version` annotations.
...
https://docs.spring.io/spring-data/commons/reference/repositories.html[Repository]
...
For a reactive variant see link:{reactive_demo_path}/simplecrud[Reactive Simple CRUD Demo].
